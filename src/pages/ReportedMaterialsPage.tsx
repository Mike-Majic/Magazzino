...
export function ReportedMaterialsPage() {
  ...
  const filtered = useMemo(() => rows.filter((r) => r.status === 'denunciato').filter((r) => (!search || [r.serial, r.model, r.sap, r.assignedTo, r.notes].join(' ').toLowerCase().includes(search.toLowerCase())) && (!fromDate || r.createdAt >= fromDate) && (!toDate || r.createdAt <= toDate)), [rows, search, fromDate, toDate]);
