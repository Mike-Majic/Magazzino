...
export function ReportedMaterialsPage() {
  ...
  const exp = () => { const csv = 'seriale,modello,sap,stato,tecnico,provenienza,note,data\n' + filtered.map((r) => `${r.serial},${r.model},${r.sap},${labels[r.status]},${r.assignedTo},${r.provenance},${r.notes},${r.createdAt}`).join('\n'); const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' })); a.download = 'denunciati.csv'; a.click(); };
