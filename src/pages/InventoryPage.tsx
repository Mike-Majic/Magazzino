...
export function InventoryPage() {
  ...
  const [scanTarget, setScanTarget] = useState<'serial' | 'sap'>('serial');
  ...
  const applySort = useCallback((list: InventoryRow[]) => {
  ...
  const visible = useMemo(() => rows
    ...
    .filter(r => (!search || [r.serial, r.model, r.sap, r.notes, r.assignedTo, r.provenance, inventoryStatusLabels[r.status]].join(' ').toLowerCase().includes(search.toLowerCase())) && (!fromDate || r.createdAt >= fromDate) && (!toDate || r.createdAt <= toDate)), [rows, search, fromDate, toDate, isTechnician, currentUserFullName]);
  const daAssegnare = useMemo(() => applySort(visible.filter(r => r.status === 'da_assegnare')), [visible, applySort]);
  const assegnati = useMemo(() => applySort(visible.filter(r => r.status === 'assegnato')), [visible, applySort]);
  ...
  const exportCsv = (list: InventoryRow[], name: string) => { const csv = buildCsv(['seriale','modello','sap','stato','assegnato','provenienza','note','data'], list.map(r => [r.serial, r.model, r.sap, inventoryStatusLabels[r.status], r.assignedTo, r.provenance, r.notes, r.createdAt])); downloadCsv(csv, name); };
          ...
          if (codes?.length) {
            ...
            if (value) {
              setNewRow((prev) => {
                if (scanTarget === 'sap') {
                  const sap = value;
                  const found = sapCatalog.find((s) => s.sapCode === sap);
                  return { ...prev, sap, model: found ? found.modelName : prev.model };
                }
                return { ...prev, serial: value };
              });
              stopScanner();
              if (scanTarget === 'serial') serialInputRef.current?.focus();
              return;
            }
          }
        } catch {
  ...
  return <section><h2>Seriali materiali (da assegnare / assegnati)</h2>
    ...
    <form className='new-modem-form' onSubmit={register}><input ref={serialInputRef} onFocus={() => { setScanTarget('serial'); setScanPromptVisible(true); }} value={newRow.serial} onChange={e => setNewRow({ ...newRow, serial: e.target.value })} placeholder='Seriale'/><input value={newRow.model} onChange={e => setNewRow({ ...newRow, model: e.target.value })} placeholder='Descrizione materiale'/><input value={newRow.sap} onFocus={() => { setScanTarget('sap'); setScanPromptVisible(true); }} onChange={e => { const sap = e.target.value.trim(); const f = sapCatalog.find(s => s.sapCode === sap); setNewRow({ ...newRow, sap, model: f ? f.modelName : '' }); }} placeholder='SAP'/><select value={newRow.status} onChange={e => setNewRow({ ...newRow, status: e.target.value as InventoryStatus })}>{inventoryStatuses.map(s => <option key={s} value={s}>{inventoryStatusLabels[s]}</option>)}</select><select value={newRow.assignedTo} onChange={e => setNewRow({ ...newRow, assignedTo: e.target.value })}><option value='-'>-</option>{users.map(u => <option key={u.id} value={`${u.firstName} ${u.lastName}`}>{u.firstName} {u.lastName}</option>)}</select><select value={newRow.provenance} onChange={e => setNewRow({ ...newRow, provenance: e.target.value })}>{companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select><input value={newRow.notes} onChange={e => setNewRow({ ...newRow, notes: e.target.value })} placeholder='Note'/><button className='action-green' type='submit'>Registra materiale</button></form>
    ...
    {scanPromptVisible && !scannerOpen && <div className='scan-prompt'><span>Vuoi scannerizzare?</span><button type='button' className='scan-prompt-yes' onClick={() => { setScanPromptVisible(false); openScanner(); }}>Si</button><button type='button' className='scan-prompt-no' onClick={() => setScanPromptVisible(false)}>No</button></div>}
    ...
    <h3>Materiali - Da assegnare</h3><div className='table-wrap'><table className='compact-table with-separators inventory-table'><thead><tr><th onClick={() => onSort('serial')}>Seriale</th><th onClick={() => onSort('model')}>Modello</th><th onClick={() => onSort('sap')}>SAP</th><th onClick={() => onSort('status')}>Stato</th><th onClick={() => onSort('assignedTo')}>Assegnato a</th><th onClick={() => onSort('provenance')}>Provenienza</th><th onClick={() => onSort('notes')}>Note</th><th>Allegati</th></tr></thead><tbody>{daAssegnare.map(r => <tr key={r.id}><td>{r.serial}</td><td>{r.model}</td><td>{r.sap}</td><td><select value={r.status} onChange={e => onBlurConfirm(r.id, 'status', e.target.value)}>{inventoryStatuses.map(s => <option key={s} value={s}>{inventoryStatusLabels[s]}</option>)}</select></td><td><select value={r.assignedTo} onChange={e => onBlurConfirm(r.id, 'assignedTo', e.target.value)}><option value='-'>-</option>{users.map(u => <option key={u.id} value={`${u.firstName} ${u.lastName}`}>{u.firstName} {u.lastName}</option>)}</select></td><td><select value={r.provenance} onChange={e => onBlurConfirm(r.id, 'provenance', e.target.value)}>{companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select></td><td><input defaultValue={r.notes} onBlur={e => onBlurConfirm(r.id, 'notes', e.target.value)} /></td><td className='attachment-cell'><button type='button' className='icon-btn' onClick={() => { attachmentRowRef.current = r.id; attachmentPickerRef.current?.click(); }}>📎</button>{(r.attachmentUrls ?? (r.attachmentUrl ? [r.attachmentUrl] : [])).slice(0,3).map((url,idx)=><span key={url+idx}><button type='button' className='icon-btn' onClick={() => window.open(url, '_blank')}>👁️</button><button type='button' className='icon-btn danger' onClick={() => deleteAttachmentAction(r, idx)}>✖️</button></span>)}<button type='button' className='icon-btn danger' onClick={() => deleteRowAction(r)}>🗑️</button></td></tr>)}</tbody></table></div>
    <h3>Materiali - Assegnati</h3><div className='table-wrap'><table className='compact-table with-separators inventory-table'><thead><tr><th onClick={() => onSort('serial')}>Seriale</th><th onClick={() => onSort('model')}>Modello</th><th onClick={() => onSort('sap')}>SAP</th><th onClick={() => onSort('status')}>Stato</th><th onClick={() => onSort('assignedTo')}>Assegnato a</th><th onClick={() => onSort('provenance')}>Provenienza</th><th onClick={() => onSort('notes')}>Note</th><th>Allegati</th></tr></thead><tbody>{assegnati.map(r => <tr key={r.id}><td>{r.serial}</td><td>{r.model}</td><td>{r.sap}</td><td><select value={r.status} onChange={e => onBlurConfirm(r.id, 'status', e.target.value)}>{inventoryStatuses.map(s => <option key={s} value={s}>{inventoryStatusLabels[s]}</option>)}</select></td><td><select value={r.assignedTo} onChange={e => onBlurConfirm(r.id, 'assignedTo', e.target.value)}><option value='-'>-</option>{users.map(u => <option key={u.id} value={`${u.firstName} ${u.lastName}`}>{u.firstName} {u.lastName}</option>)}</select></td><td><select value={r.provenance} onChange={e => onBlurConfirm(r.id, 'provenance', e.target.value)}>{companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select></td><td><input defaultValue={r.notes} onBlur={e => onBlurConfirm(r.id, 'notes', e.target.value)} /></td><td className='attachment-cell'><button type='button' className='icon-btn' onClick={() => { attachmentRowRef.current = r.id; attachmentPickerRef.current?.click(); }}>📎</button>{(r.attachmentUrls ?? (r.attachmentUrl ? [r.attachmentUrl] : [])).slice(0,3).map((url,idx)=><span key={url+idx}><button type='button' className='icon-btn' onClick={() => window.open(url, '_blank')}>👁️</button><button type='button' className='icon-btn danger' onClick={() => deleteAttachmentAction(r, idx)}>✖️</button></span>)}<button type='button' className='icon-btn danger' onClick={() => deleteRowAction(r)}>🗑️</button></td></tr>)}</tbody></table></div>
  </section>;
}
