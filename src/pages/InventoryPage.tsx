...
export function InventoryPage() {
  ...
  return <section><h2>Seriali modem (da assegnare / assegnati)</h2>
    ...
    <div className='filters-row'><input className='search-input' value={search} onChange={e=>setSearch(e.target.value)} placeholder='Cerca...' /><input type='date' value={fromDate} onChange={e=>setFromDate(e.target.value)}/><input type='date' value={toDate} onChange={e=>setToDate(e.target.value)}/><button className='icon-btn' onClick={()=>exportCsv(visible,'magazzino.csv')}>Export Excel/CSV</button></div>
