...
export function InventoryPage() {
  ...
  return <section><h2>Seriali materiali (da assegnare / assegnati)</h2>
    ...
    <form className='new-modem-form' onSubmit={register}><input ref={serialInputRef} onFocus={() => { setScanTarget('serial'); setScanPromptVisible(true); }} value={newRow.serial} onChange={e => setNewRow({ ...newRow, serial: e.target.value })} placeholder='Seriale'/><input value={newRow.model} onChange={e => setNewRow({ ...newRow, model: e.target.value })} placeholder='Descrizione materiale'/><input value={newRow.sap} onFocus={() => { setScanTarget('sap'); setScanPromptVisible(true); }} onChange={e => { const sap = e.target.value.trim(); const f = sapCatalog.find(s => s.sapCode === sap); setNewRow({ ...newRow, sap, model: f ? f.modelName : '' }); }} placeholder='SAP'/><select value={newRow.status} onChange={e => setNewRow({ ...newRow, status: e.target.value as InventoryStatus })}>{statuses.map(s => <option key={s} value={s}>{labels[s]}</option>)}</select><select value={newRow.assignedTo} onChange={e => setNewRow({ ...newRow, assignedTo: e.target.value })}><option value='-'>-</option>{users.map(u => <option key={u.id} value={`${u.firstName} ${u.lastName}`}>{u.firstName} {u.lastName}</option>)}</select><select value={newRow.provenance} onChange={e => setNewRow({ ...newRow, provenance: e.target.value })}>{companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select><input value={newRow.notes} onChange={e => setNewRow({ ...newRow, notes: e.target.value })} placeholder='Note'/><button className='action-green' type='submit'>Registra materiale</button></form>
