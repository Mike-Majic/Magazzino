...
type SetupTab = 'users' | 'companies' | 'sap';

export function SetupPage() {
  const [activeTab, setActiveTab] = useState<SetupTab>('users');
  ...
  const removeUser=(id:number)=>{const c=users.find(u=>u.id===id); if(!c||c.locked||c.email?.toLowerCase()===SUPERADMIN_EMAIL) return; persistUsers(users.filter(u=>u.id!==id));};
  const resetPassword=(id:number)=>{const c=users.find(u=>u.id===id); if(!c||c.email?.toLowerCase()===SUPERADMIN_EMAIL) return; const next=prompt('Nuova password'); if(!next) return; updateUser(id,{password:next});};
  const toggleLock=(id:number)=>{const c=users.find(u=>u.id===id); if(!c||c.email?.toLowerCase()===SUPERADMIN_EMAIL) return; persistUsers(users.map(u=>u.id===id?{...u,locked:!u.locked}:u));};
  ...
  return <section><div className='section-header'><h2>Setup progetto</h2><p>Gestisci una categoria alla volta per mantenere la pagina più ordinata.</p></div><div className='setup-tabs' role='tablist' aria-label='Categorie setup'><button type='button' className={activeTab === 'users' ? 'active' : ''} onClick={() => setActiveTab('users')}>Utenti</button><button type='button' className={activeTab === 'companies' ? 'active' : ''} onClick={() => setActiveTab('companies')}>Imprese</button><button type='button' className={activeTab === 'sap' ? 'active' : ''} onClick={() => setActiveTab('sap')}>Modelli/SAP</button></div>{activeTab === 'users' && <div className='setup-panel'><h3>Utenze (aggiungi / modifica / elimina)</h3>
