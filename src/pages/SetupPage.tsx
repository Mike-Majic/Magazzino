...
export function SetupPage() {
  ...
  useEffect(() => { const u=localStorage.getItem('users_registry'); const s=localStorage.getItem('sap_catalog'); const c=localStorage.getItem('companies_registry'); if(u) setUsers(JSON.parse(u)); if(s) setSapCatalog(JSON.parse(s)); if(c) setCompanies(JSON.parse(c)); }, []);
  ...
  const addCompany=(e:FormEvent)=>{e.preventDefault(); if(!companyName.trim()) return; const next=[...companies,{id:Date.now(),name:companyName.trim()}]; setCompanies(next); localStorage.setItem('companies_registry',JSON.stringify(next)); setCompanyName('');};
  const updateCompany=(id:number,name:string)=>{const next=companies.map(c=>c.id===id?{...c,name}:c); setCompanies(next); localStorage.setItem('companies_registry',JSON.stringify(next));};
  const removeCompany=(id:number)=>{const next=companies.filter(c=>c.id!==id); setCompanies(next); localStorage.setItem('companies_registry',JSON.stringify(next));};
