...
export function App() {
  ...
  if (!activeUser) return <main className='login-page'><div className='login-card dark-login'><h2>Accesso</h2><form onSubmit={login} className='login-form'><label>Email</label><input placeholder='nome@email.it' type='email' value={email} onChange={(e)=>setEmail(e.target.value)} /><label>Password</label><div className='password-wrap'><input placeholder='Password' type='password' value={password} onChange={(e)=>setPassword(e.target.value)} /></div>{!existing && <><input placeholder='Nome' value={firstName} onChange={e=>setFirstName(e.target.value)} /><input placeholder='Cognome' value={lastName} onChange={e=>setLastName(e.target.value)} /></>}<div className='login-grid'><button type='submit' className='btn-green'>Login</button>{!existing && <button type='button' className='btn-orange' onClick={register}>Compila registrazione</button>}<button type='button' className='btn-blue' onClick={resetPassword}>Richiedi reset password</button><button type='button' className='btn-gray' onClick={()=>window.alert('Contatta admin di sistema')}>AIUTO</button></div></form></div></main>;

  return <div className='layout'><aside className='sidebar'><BrandLogo compact /><h1>Magazzino</h1><nav>{navItems.map((item)=><NavLink key={item.to} to={item.to} className={({isActive})=>(isActive?'active':'')}>{item.label}</NavLink>)}</nav></aside><main className='content'><div className='top-userbar'><BrandLogo compact /><span>{activeUser.firstName} {activeUser.lastName}</span><button className='icon-btn' onClick={logout}>Logout</button></div><Routes><Route path='/' element={<DashboardPage />} /><Route path='/inventory' element={<InventoryPage />} /><Route path='/movements' element={<MovementsPage />} /><Route path='/installed' element={<InstalledModemsPage />} /><Route path='/to-return' element={<ToReturnModemsPage />} /><Route path='/reported' element={<ReportedMaterialsPage />} /><Route path='/setup' element={<SetupPage />} /></Routes></main></div>;
}
